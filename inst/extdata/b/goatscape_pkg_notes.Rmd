...
* add support for other input formats; shp? sp?
...
View(censusapi::listCensusMetadata(name="sf3", vintage = 1990, type = "variables"))
View(censusapi::listCensusMetadata(name="sf3", vintage = 2000, type = "variables"))
View(censusapi::listCensusMetadata(name="sf1", vintage = 2010, type = "variables"))


### get_fips
* to get the census data we need the state and county fips codes
* the [tigris] package allows us to download the tiger files for line work for each year.  Linework available in two resolutions
    - low resolution generalized (1:500k); Code: counties(cb=FALSE, year = 2010)
    - high resolution Tiger files; Code: counties(cb=FALSE, year = 2010)
        - NOTE: for cb = false there should be files availabe at 5m and 20m resolution; both options return the same file.
